...
def MISLNet(x,phase,nprefilt=6,reuse=False, namescope='MISLNet'):
    with tf.variable_scope(namescope, reuse=reuse) as scope:
...
def MISLNet_pre(x, cls_num, phase, nprefilt=6,reuse=False, namescope='MISLNet'):
    with tf.variable_scope(namescope, reuse=reuse) as scope:
        w_constr = weight_variables(name='weights_cstr', shape=[5, 5, 3, nprefilt])
        constr_output = conv2d(x, w_constr, pad='VALID')
        ...
        w_fc3 = weight_variables(name='weights_d3', shape=[200, cls_num])
        b_fc3 = tf.get_variable(name='bias_d3', shape=[cls_num], dtype=tf.float32, initializer=tf.constant_initializer(0))
        fc3 = tf.add(tf.matmul(fc2, w_fc3), b_fc3)

    return fc3

def CompareNet(x1,x2,phase,nprefilt=6,nb12=2048,nb3=64,reuse=False):

    MISL_output1 = MISLNet(x1, phase, nprefilt=nprefilt,reuse=reuse, namescope='MISLNet') #feature extractor for patch 1
    MISL_output2 = MISLNet(x2, phase, nprefilt=nprefilt,reuse=True, namescope='MISLNet') #feature extractor for patch 2, with same weights as 1

    with tf.variable_scope('CompareNet',reuse=reuse) as scope:
        ...
        if phase:
            ...
            fcb1_drop = tf.nn.dropout(fcb1,rate=0)

        fcb2 = tf.nn.relu(tf.add(tf.matmul(MISL_output2, w_fcb1), b_fcb1),name='fcb2_out')
        if phase:
            fcb2_drop = tf.nn.dropout(fcb2,rate=0.5)
        else:
            fcb2_drop = tf.nn.dropout(fcb2,rate=0)
        fcb1b2_mult = tf.multiply(fcb1_drop,fcb2_drop,name='fcb1b2_mult')
        fcb1b2_concat = tf.concat([fcb1_drop,fcb1b2_mult,fcb2_drop],1)
...
def FeatCompareNet(f1,f2,phase,nb12=2048,nb3=64,reuse=False):
    ...
    with tf.variable_scope('CompareNet',reuse=reuse) as scope:
        ...
        if phase: #if training, use dropout
            fcb1_drop = tf.nn.dropout(fcb1,rate=0.5)
        else: #if not training, use all connections
            fcb1_drop = tf.nn.dropout(fcb1,rate=0)
