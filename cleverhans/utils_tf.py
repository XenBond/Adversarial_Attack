...
def tf_model_load_namescope(sess, file_path=None, local_namescope=None, model_namescope=None):
  # import model under new namescope with old namescope
  with sess.as_default():
    var_list = [v for v in tf.global_variables() if local_namescope in v.name]
    model_var = []
    for ii, _ in enumerate(var_list):
        model_var.append((var_list[ii].name.replace(local_namescope, model_namescope)).replace(':0', ''))
    final_var_list = dict(zip(model_var, var_list))
    saver = tf.train.Saver(var_list=final_var_list)
    if file_path is None:
      error = 'file_path argument is missing.'
      raise ValueError(error)
    saver.restore(sess, file_path)
